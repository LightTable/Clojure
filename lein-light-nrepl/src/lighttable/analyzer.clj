...
(ns lighttable.analyzer
  "Fork of cljs.analyzer used by instarepl. Formerly known as ibdknox.analyzer -
  https://clojars.org/ibdknox/analyzer"
                             ...
                             (if (symbol? spec)
                               ...
                               (let [[lib & opts] spec
                                 ...
                                 (assert (or (and (sequential? referred) (every? symbol? referred))
                                             (= :all referred)
                                             (nil? referred))
                                         (error-msg spec ":refer must be followed by a sequence of symbols or :all in :require / :require-macros"))
                                 (when-not macros?
                                   (swap! deps conj lib))
                                 (merge (when alias {rk {alias lib}})
                                        (when (and referred (not= referred :all)) {uk (apply hash-map (interleave referred (repeat lib)))})))))
        use->require (fn use->require [spec]
                       (let [[lib kw referred :as spec] (if (coll? spec)
                                spec
                                [spec])]
